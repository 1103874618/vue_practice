$(document).ready(function () {
  const POMODORO_STATES = {
    WORK: 'work',
    REST: 'rest'
  };
  const WORKING_TIME_LENGTH_IN_MINUTES = 1;
  const RESTING_TIME_LENGTH_IN_MINUTES = 5;
  const STATES = {
    STARTED: 'started',
    STOPPED: 'stopped',
    PAUSED: 'paused'
  };

  
  

  new Vue({
    el: '#app',
    data: {
      state: STATES.STOPPED,
      minute: WORKING_TIME_LENGTH_IN_MINUTES,
      second: 0,
      pomodoroState: POMODORO_STATES.WORK,
      timestamp: 0
    },

    computed: {
      title: function () {
        return this.pomodoroState === POMODORO_STATES.WORK ? 'Work!' :
          'Rest!'
      },
      //时间分钟二级处理
      min: function () { //时间空隙填充0
        if (this.minute < 10) {
          return '0' + this.minute;
        }
        return this.minute;
      },
      sec: function () {
        if (this.second < 10) {
          return '0' + this.second;
        }
        return this.second;
      }
    },


    methods: {
      show_state: function () { 
          
       },
      start: function () {
        this.state = STATES.STARTED;
        this._tick();
        this.interval = setInterval(this._tick, 1000);
        console.log(this.pomodoroState);
      },
      pause: function () {
        this.state = STATES.PAUSED;
        clearInterval(this.interval);
      },
      stop: function () {
        this.state = STATES.STOPPED;
        clearInterval(this.interval); //先停止
        this.pomodoroState = POMODORO_STATES.WORK; //后复位
        this.minute = WORKING_TIME_LENGTH_IN_MINUTES; //??为什么只有工作状态的时间
        this.second = 0;
      },

      _tick: function () { //自减函数

        if (this.second % 10 === 0) {
          this.timestamp = new Date().getTime();
        }
        //if second is not 0, just decrement second
        if (this.second !== 0) {
          this.second--;
          return;
        }
        //if second is 0 and minute is not 0,
        //decrement minute and set second to 59
        if (this.minute !== 0) {
          this.minute--;
          this.second = 59;
          return;
        }
        //if second is 0 and minute is 0,
        //toggle working/resting intervals
        this.pomodoroState = this.pomodoroState ===
          POMODORO_STATES.WORK ? POMODORO_STATES.REST :
          POMODORO_STATES.WORK;

        if (this.pomodoroState === POMODORO_STATES.WORK) {
          this.minute = WORKING_TIME_LENGTH_IN_MINUTES;
        } else {
          this.minute = RESTING_TIME_LENGTH_IN_MINUTES;
        }
      }
    }

  });

  
});

var settings = {
  "async": true,
  "crossDomain": true,
  "url": "https://api.thecatapi.com/v1/images/search?format=json",
  "method": "GET",
  "headers": {
    "Content-Type": "application/json",
    "x-api-key": "0246a4db-d137-4cff-ab47-0d5f7ab95005"
  }
}

/* $.ajax(settings).done(function (response) {
  $(".container").append($("<img src= " + response[0].url + ">"));
  console.log(response);
});
 */
